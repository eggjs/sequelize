...
const ContextModelMap = Symbol('ContextModelMap');
...
class Model {
  ...
  /**
   * Create a ContextModel
   *
   * @param {Context} ctx    Context style class instance, like koa Context instance
   * @return {ContextModel}  A ContextModel extends from normal Model, it contains `ctx` property.
   */
  static contextify(ctx) {
    const _Model = this;
    if (!ctx[ContextModelMap]) {
      ctx[ContextModelMap] = new Map();
    }
    const klass = ctx[ContextModelMap].get(_Model.name);
    if (klass) return klass;

    class ContextModel extends _Model {
      // constructor(...args) {
      //   super(...args);
      // }
    };

    Object.defineProperty(ContextModel, 'name', { value: _Model.name });
    ContextModel.ctx = ContextModel.prototype.ctx = ctx;
    ContextModel.isContextModel = true;
    ctx[ContextModelMap].set(_Model.name, ContextModel);

    // patch associations
    const associations = ContextModel.associations;
    const keys = Object.keys(associations);
    ContextModel.associations = {};
    for (const associationAccessor of keys) {
      // Contextify all assocation Models first
      const association = Object.create(associations[associationAccessor]);
      if (association.source) {
        association.source = association.source.contextify(ctx);
      }
      if (association.target) {
        association.target = association.target.contextify(ctx);
      }

      // can skip injectAttributes on association
      // association.injectAttributes();

      // cleanup association.accessors on association.source.prototype
      for (const k in association.accessors) {
        const method = association.accessors[k];
        if (association.source.prototype[method] && association.source.prototype[method].__mixinMethod) {
          association.source.prototype[method] = undefined;
        }
      }
      // can't skip mixin: becuase it apply association on Model instance's accessors methods, like `getXXX, createXXX, setXXX`
      association.mixin(association.source.prototype);

      ContextModel.associations[associationAccessor] = association;
    }
