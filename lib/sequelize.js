...
class Sequelize {
  ...
  query(sql, options) {
    ...
    return Promise.try(() => {
      ...
      const connectDuration = Date.now() - begin;
      const query = new this.dialect.Query(connection, this, options);
      const retryOptions = _.extend(this.options.retry, options.retry || {});

      this.runHooks('beforeQuery', sql, { connectDuration }, options);
      return retry(() => query.run(sql, bindParameters), retryOptions)
        .then(results => {
          const duration = Date.now() - begin - connectDuration;
          this.runHooks('afterQuery', sql, { results, error: null, duration, connectDuration }, options);
          return results;
        }, error => {
          const duration = Date.now() - begin - connectDuration;
          this.runHooks('afterQuery', sql, { results: null, error, duration, connectDuration }, options);
          throw error;
        })
        .finally(() => {
